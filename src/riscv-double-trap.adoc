...
to 0. If it is VU, then `vsstatus.SDT` is also set to 0.
...
When a hart resumes from Debug Mode, if the new privilege mode is not M-mode,
then `MDT` bit is set to 0. If the new privilege mode is U, VS, or VU, then
`sstatus.SDT` is also set to 0. Additionally, if it is VU, then `vsstatus.SDT`
is set to 0.
