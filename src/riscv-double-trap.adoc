...
In S-mode, the `SRET` instruction sets `sstatus.SDT` to 0, and in VS-mode, it
sets `vsstatus.SDT` to 0. If the new privilege mode is less privileged than
VS-mode, it also sets `vsstatus.SDT` to 0.
...
When a hart resumes from Debug Mode, if the new privilege mode is less privileged
than M-mode, `mstatus.MDT` is set to 0. If it is less privileged than S/HS-mode,
`sstatus.SDT` is set to 0. If it is less privileged than VS-mode, `vsstatus.SDT`
is set to 0.
