...
[NOTE]
====
After a trap handler, such as `scause`, `sepc`, and `stval` 
among others, saves the state that is needed to resume from the trap, it
should clear the `SDT` bit when it is reentrant.

If the `SDT` is reset by an `SRET`, the trap handler is enabled to detect double
traps that occur during the tail phase, where it restores critical state to return
from a trap.
...
[NOTE]
====
The actions performed by the platform when a har asserts a `critical-error` signal
...
critical-error signal to the platform. When the hart enters Debug Mode, the
...
highest priority among all reasons for entering Debug Mode. Resuming operations from 
Debug Mode following an entry from the critical error state returns the hart to the
critical error state.

[NOTE]
====
The implication of this specification is that resuming operations from Debug Mode,
...
it will cause the critical-error signal to be sent to the platform.  The
...
hart or resetting the platform by using the Debug Module reset control.
====
