...
critical error state. When a hart resumes from Debug Mode, if the new privilege
mode is less privileged than M-mode, `mstatus.MDT` is set to 0. If it is less
privileged than S/HS-mode, `sstatus.SDT` is set to 0. If it is less privileged
than VS-mode, `vsstatus.SDT` is set to 0.
