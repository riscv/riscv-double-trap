...
with `mnstatus.NMIE` set to 0 is an _unexpected trap_. A trap caused by an RNMI
is not considered an _unexpected trap_. The _unexpected trap_ is handled as
follows:
