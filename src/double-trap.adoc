...
`vsstatus` CSR and a `DTE` field (bit 59) to the `henvcfg` CSR. For RV32, the
`DTE` field is located in bit 27 of the `menvcfgh` and `henvcfgh` CSRs. The
Ssdbltrap extension requires the implementation of the `mtval2` CSR. An Exception
Code (value = 16) called the double trap is introduced. The bit 16 of the `medeleg`
and the `hedeleg` CSRs are read-only zero as the double trap is not delegatable.

When `menvcfg(h).DTE` is zero, the implementation behaves as though Ssdbltrp is not
implemented. When Ssdbltrp is not implemented `mstatus.SDT`, `vsstatus.SDT`, and
`henvcfg(h).DTE` bits are read-only zero. When `henvcfg(h).DTE` is zero, the
