...
Code (value = 16) called the double trap is introduced. The bit 16 of the
`medeleg` and the `hedeleg` CSRs are read-only zero as the double trap is not
delegatable. The Ssdbltrap extension requires the implementation of the `mtval2`
CSR.
...
already set to 1, a critical error condition occurs. The trap that caused the
