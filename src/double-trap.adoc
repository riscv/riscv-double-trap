...
CSR at bit position 7. Upon reset, the `MDT` field is set to 1. The Ssdbltrp 
...
field of `dcsr` is set to 1, and `dpc` set to the `pc` at which the interrupt or
the exception causing the unexpected trap occurred. Resuming from Debug Mode
with `pcerr` is 1 returns the hart to the critical error state, and the `pc`
following this resume UNSPECIFIED.
