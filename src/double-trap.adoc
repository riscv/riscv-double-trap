...
When the `SDT` bit is set to 1, the `SIE` bit is automatically set to 0,
regardless of the value written to the `SIE` bit. While the `SDT` bit is 1, the
`SIE` bit is read-only zero. When the `SDT` bit is cleared, `SIE` bit assumes
the value written to it. When the `MDT` bit is set to 1, the `MIE` bit is
automatically set to 0, regardless of the value written to the `MIE` bit. While
the `MDT` bit is 1, the `MIE` bit is read-only zero. When `MDT` bit is cleared,
the `MIE` bit remains zero in RV32, and assumes the value written to it in RV64.
