...
The Smdbltrp extension introduces an disable-traps-for-M-mode (`DTM`) bit to the
`mstatus` CSR.

The Ssdbltrp adds an disable-traps-for-S-mode (`DTS`) bit to the `mstatus` CSR
and a double-trap-enable (`DTE`) bit to the `menvcfg` CSR. Ssdbltrp also adds a
`DTS` bit to the `vsstatus` CSR and a `DTE` bit to the `henvcfg` CSR if the
...
`mstatus.DTS`, `vsstatus.DTS`, and `henvcfg.DTE` bits are read-only zero. When
`henvcfg.DTE` is zero, the implementation behaves as though Ssdbltrp is not
implemented for VS-mode and the `vsstatus.DTS` bit is read-only zero.


When a trap is taken into privilege mode `x`, the `TD__x__` bit is set to 1 if
it was not already set. However, if `TD__x__` is already set to 1, a double trap
...
[NOTE]
====
The trap handler should clear the `TD__x__` bit only after it has saved the
...
When `TD__x__` is 1, interrupts delegated to privilege mode `x` are globally
disabled.
...
global interrupt masking is activated, by setting `TD__x__`, to prevent pending
...
first clearing `TD__x__`. Such an error could lead to a pending virtual
...
globally masked when `TD__x__` is set to 1.
