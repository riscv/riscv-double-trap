...
[NOTE]
====
A trap handler after saving the state necessary for resuming from the trap,
including `__x__cause`, `__x__epc`, and `__x__tval` among others, should clear the
`__x__DT` bit when it becomes reentrant.
====
...
-- albeit with added effort -- by software through the process of walking the
page tables.

For a double trap to M-mode originating from a critical error in VS-mode, M-mode
should redirect the exception to HS-mode by copying the values of M-mode CSRs
updated by the double trap to corresponding HS-mode CSRs and should use an `MRET`
to resume execution in HS-mode at the address in `stvec`.
